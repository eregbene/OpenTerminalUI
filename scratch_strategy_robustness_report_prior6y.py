"""Same as scratch_strategy_robustness_report_prior3y.py, but for years 6-9 ago -- third and
(given the corpus is ~8 years deep) likely final window in this recency-window comparison."""
...
WINDOW_START_YEARS_AGO = 9
WINDOW_END_YEARS_AGO = 6
SAMPLE_CAP = 8000
BOOTSTRAP_PATHS = 500
...
def main() -> None:
    ...
    for strategy in STRATEGIES:
        ...
        if not rows:
            print(f"{strategy}: n=0 (no data in this window)", flush=True)
            full_reports[strategy] = {"strategy": strategy, "sample_size": 0, "note": "no data in this window"}
            rows_out.append({"strategy": strategy, "trades": 0, "gross_exp": None, "net_exp": None, "cost_drag": None,
                              "pf_gross": None, "pf_net": None, "oos_net_exp": None, "psr": None, "dsr": None,
                              "ess": 0, "mc_p95_dd": None, "verdict": "NO_DATA", "wf_verdict": None})
            continue
    ...
    print("\n\n=== MARKDOWN TABLE (prior 6-9 years ago) ===\n")
    ...
    with open("/app/scratch_strategy_robustness_prior6y_full.json", "w") as f:
        json.dump(full_reports, f, indent=2, default=str)
    print("\nfull JSON written to /app/scratch_strategy_robustness_prior6y_full.json", flush=True)
