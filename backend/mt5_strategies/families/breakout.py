...
3 restricted strategies), stale-exit metadata, and the spread safety buffer. With every flag at
its default, `evaluate_breakout` is behavior-identical to the pre-restructure implementation --
same rejection reasons, same evidence keys, same strength formula, same stop/target math. Nothing
here changes risk-per-trade, stop/target geometry formulas, or the minimum reward:risk floor
(_shared._MIN_REWARD_MULTIPLE, untouched).
"""
...
def _build_signal(ctx: StrategyContext, *, direction: str, entry: Decimal, structure_ref: Decimal | float | None,
    ...
    evidence = {**evidence, "consolidation_quality_score": consolidation_score, "market_regime": ctx.market_regime}
