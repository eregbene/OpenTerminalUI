...
# prevent runaway *profit-taking* spam (see VOLUME_MUTATING_COOLDOWN_ACTION_TYPES's incident
# history) should never be the reason a losing/invalidated/contaminated position couldn't be
# cut. Audited every existing full/partial-close action type against two criteria -- (a) always
# closes 100% of the position, not a fraction, and (b) triggered by an adverse/risk condition,
# not by profit reaching a target -- and kept this list to exactly the two that pass both:
#   - THESIS_INVALIDATION_CLOSE: always full exit (see _v2_candidates/_evaluate_position),
#     triggered purely by adverse price action invalidating the setup -- the flagship case.
#   - VALIDATION_INCIDENT_CLOSE: always full exit, operator-triggered via
#     close_incident_position() for a contaminated/risk-metadata-incident position -- an
#     explicit human emergency action, arguably the most urgent of all; being silently
#     rate-limited by unrelated autonomous activity earlier in the hour would defeat its
#     purpose. Low spam risk since it is never auto-generated by the monitor loop.
# Deliberately NOT included, with reasoning:
#   - EVENT_RISK_REDUCTION / ECONOMIC_REDUCE_SIZE: reduce exposure by a FRACTION, not a full
#     exit -- prudent size reduction ahead of/during event risk, not an emergency close.
#   - MFE_PROTECTION_CLOSE / TP_PROGRESS_PROFIT_LOCK / TP_PROGRESS_PARTIAL_PROTECT /
#     PARTIAL_PROFIT: all fundamentally profit-management (protecting or locking in gains
#     already made), exactly the "normal/profit-management" category the cap must keep
#     governing per this change's own requirement.
#   - TIME_EXIT: always a full exit, but not an emergency -- fires only on stagnation
...
class AdaptiveManagementService:
    ...
    async def _monitor_cycle(self) -> dict[str, Any]:
        ...
        async with self._cycle_lock:
            ...
            # one shared id per _monitor_cycle invocation, threaded through to every
            # per-position observation row this cycle. Purely a grouping key; never read by
    ...
    def _can_execute(self, action: AdaptiveManagementActionORM, state: AdaptivePositionStateORM, activation: Any | None, breaker: Any, mode: str, current_account_fingerprint: str | None = None) -> bool:
        ...
        # bypass ADAPTIVE_MAX_ACTIONS_PER_HOUR entirely -- deliberately never call
        # _rate_limit_ok for them, so they neither consume nor are blocked by the shared hourly
        # budget other (normal/profit-management) actions use. Every other gate above and below
        # this line -- mode, activation, circuit breaker, live/DEMO, account classification,
        # fingerprint, cooldown, eligible symbols -- still applies unchanged.
        if action.action_type not in RATE_LIMIT_EXEMPT_ACTION_TYPES and not _rate_limit_ok(breaker, activation.maximum_actions_per_hour):
            return False
        symbols = [str(row).upper() for row in (activation.eligible_symbols or [])]
        return not symbols or state.symbol in symbols

    async def _check_position_freshness(self, action: AdaptiveManagementActionORM, state: AdaptivePositionStateORM) -> str | None:
        """Re-fetches the live MT5 position immediately before a broker mutation is built, and
        verifies it still matches the snapshot this action's candidate/request was computed
        from. `payload`/`state` are captured once at the START of the monitor cycle; real I/O
        (context/candle/economic-intelligence fetches) happens between then and here, so the
        position can have closed, been modified by another process, or hit its SL/TP naturally
        in the meantime. Returns a specific stale-state reason (never submitted in that case)
        instead of relying on broker-side rejection alone -- a stale PARTIAL_PROFIT could close
        ...
        # Defense-in-depth beyond the position_id lookup above: state.broker_ticket is recorded
        # independently (see _sync_position_state) and must still agree with what the broker
        # reports for this same position -- if it doesn't, this cycle's snapshot is not trusted.
        fresh_ticket = str(fresh.get("ticket") or fresh.get("identifier") or "")
        if state.broker_ticket and fresh_ticket and fresh_ticket != str(state.broker_ticket):
            return "STALE_POSITION_TICKET_MISMATCH"
        if str(fresh.get("symbol") or "").upper() != str(state.symbol or "").upper():
            return "STALE_POSITION_SYMBOL_MISMATCH"
        if _position_direction(fresh) != state.direction:
            return "STALE_POSITION_DIRECTION_MISMATCH"
        if action.action_type in VOLUME_CLOSE_ACTION_TYPES:
            fresh_volume = float(fresh.get("volume") or 0)
            expected_volume = float(state.current_volume or 0)
            if expected_volume > 0 and not _price_matches(expected_volume, fresh_volume, tolerance_fraction=0.001):
                return "STALE_POSITION_VOLUME_CHANGED"
        if action.action_type in SLTP_MODIFY_ACTION_TYPES:
            fresh_sl = fresh.get("sl")
            fresh_tp = fresh.get("tp")
            expected_sl = state.current_sl
            expected_tp = state.current_tp
            sl_matches = _price_matches(float(expected_sl), float(fresh_sl)) if expected_sl and fresh_sl else (not expected_sl and not fresh_sl)
            tp_matches = _price_matches(float(expected_tp), float(fresh_tp)) if expected_tp and fresh_tp else (not expected_tp and not fresh_tp)
            if not sl_matches or not tp_matches:
                return "STALE_POSITION_SLTP_CHANGED"
        return None
    ...
    async def _build_mt5_request(self, mt5: Any, symbol: Any, quote: Any, action: AdaptiveManagementActionORM, state: AdaptivePositionStateORM, payload: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if action.action_type in SLTP_MODIFY_ACTION_TYPES:
            ...
            if not self._sltp_respects_broker_distance(symbol, quote, state.direction, sl, tp):
                # Proactive floor, matching the entry-side policy (validate BEFORE submission
                # rather than rely on order_send rejection): skip this cycle as a NOOP so a
                # fresh candidate can retry once price moves clear of the broker's stop/freeze
                # zone, instead of submitting a request the broker would reject anyway.
                return None
            return {
    ...
    def _sltp_respects_broker_distance(self, symbol: Any, quote: Any, direction: str, sl: float | None, tp: float | None) -> bool:
        """Part 17: SL/TP modifications must respect the broker's minimum stop/freeze distance
        from the CURRENT price before submission -- mirrors the entry-side geometry policy
        (validate before order_send, don't rely on broker rejection). Uses whichever of
        trade_stops_level/trade_freeze_level is larger, matching how MT5 enforces both."""
        stops_level = float(getattr(symbol, "trade_stops_level", None) or 0)
        freeze_level = float(getattr(symbol, "trade_freeze_level", None) or 0)
        ...
        if min_distance <= 0:
            return True
        price = float(quote.bid if direction == "LONG" else quote.ask)
